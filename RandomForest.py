...
plt.plot(train_sizes, train_mae, 'o-', color='blue', linewidth=2, markersize=8,
         label='Обучающая выборка')
plt.plot(train_sizes, val_mae, 'o-', color='red', linewidth=2, markersize=8,
         label='Валидационная выборка (CV)')
...
# 6. Финальная оценка
print("\n" + "="*50)
print("ФИНАЛЬНАЯ ОЦЕНКА RandomForest НА ТЕСТОВОЙ ВЫБОРКЕ:")
print("="*50)
...
print("="*50)
print(f"MSE на тесте: {metrics.mean_squared_error(y_test_final, y_pred_final):.6f}")

# 8. Примеры предсказаний (опционально)
results = pd.DataFrame({
    "y_true": y_test_final,
    "y_pred": y_pred_final,
    "error": np.abs(y_test_final - y_pred_final)
})
print("\nПримеры предсказаний (первые 5 строк):")
print(results.head())
print(f"\nСредняя ошибка: {results['error'].mean():.4f}")
print(f"Максимальная ошибка: {results['error'].max():.4f}")